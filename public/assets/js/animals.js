...
const getAnimals = (formData = {}) => {
  ...
  fetch(queryUrl) // fetch get request for server /api/animals
    .then((response) => {
      if (!response.ok) {
        return alert(`error: ${response.statusText}`); // for public, display as modal or a "toast" dialogbox
      }
      return response.json();
    })
    .then((animalsData) => {
      console.log(animalsData);
      printResults(animalsData);
    });
};
